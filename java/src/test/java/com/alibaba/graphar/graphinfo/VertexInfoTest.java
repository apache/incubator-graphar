...
public class VertexInfoTest {
  ...
  public void test1() {
    ...
    PropertyGroup propertyGroup3 = PropertyGroup.factory.create(propertyStdVector2, FileType.CSV);
    ...
    Assert.assertEquals(property.isPrimary(), vertexInfo.isPrimaryKey(property.getName()).value());
    ...
    Assert.assertTrue(
        property.getName().eq(propertyGroupResult.value().getProperties().get(0).getName()));
    ...
    Assert.assertEquals(
        vertexInfo.getPrefix().toJavaString() + "vertex_count", maybePath2.value().toJavaString());
