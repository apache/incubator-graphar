...
public class EdgeChunkWriterTest {
  ...
  public void test1() {
    ...
    StdString edgeMetaFile = StdString.create(root + "/ldbc_sample/csv/person_knows_person.edge.yml");
    ...
    EdgeChunkWriter writer = EdgeChunkWriter.factory.create(edgeInfo, StdString.create("/tmp/"), AdjListType.ordered_by_source);
    ...
    Assert.assertTrue(writer
            .sortAndWriteAdjListTable(table, 0, 0, writer.getValidateLevel()).ok());
    // Write number of edges for vertex chunk 0
    Assert.assertTrue(writer.writeEdgesNum(0, table.get().num_rows(), writer.getValidateLevel()).ok());
