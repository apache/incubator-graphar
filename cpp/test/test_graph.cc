...
TEST_CASE_METHOD(GlobalFixture, "Graph") {
  ...
  SECTION("EdgesCollection") {
    ...
    // EdgeIter sync tests - prevent regression of property misalignment bug
    ...
    // Bug: segmented iteration could return different results than sequential
    std::vector<std::string> reference_dates;
    std::vector<IdType> reference_sources;
    auto it_ref = edges2->begin();
    const size_t SYNC_TEST_SIZE =
        std::min(static_cast<size_t>(50), edges2->size());

    // Collect reference data with sequential iteration
    for (size_t i = 0; i < SYNC_TEST_SIZE && it_ref != edges2->end();
         ++i, ++it_ref) {
      reference_dates.push_back(
          it_ref.property<std::string>("creationDate").value());
    ...
    for (size_t target_pos = 0; target_pos < SYNC_TEST_SIZE; ++target_pos) {
      ...
      for (size_t pos = 0; pos < target_pos && it_seg != edges2->end();
           ++pos, ++it_seg) {}

      if (it_seg != edges2->end()) {
        REQUIRE(it_seg.property<std::string>("creationDate").value() ==
                reference_dates[target_pos]);
        REQUIRE(it_seg.source() == reference_sources[target_pos]);
      }
    }

    // Test 2: Multiple property access consistency
    // Bug: repeated property() calls could return different values
    ...
    // Bug: it.property() and (*it).property() could diverge
    auto it_consistency = edges2->begin();
    for (int i = 0; i < 20 && it_consistency != edges2->end();
         ++i, ++it_consistency) {
      auto iter_date =
          it_consistency.property<std::string>("creationDate").value();
    ...
    // Bug: shared property readers could cause cross-contamination
    ...
    if (it1 != edges2->end() && it2 != edges2->end()) {
      ...
      // Access it2 properties (this could affect it1 if readers shared)
      volatile auto temp_date2 =
          it2.property<std::string>("creationDate").value();
    ...
    // Bug: crossing chunk boundaries could cause sync issues
    auto it_boundary = edges2->begin();
    std::string last_date;
    size_t boundary_count = 0;
    const size_t MAX_BOUNDARY_TEST =
        std::min(static_cast<size_t>(100), edges2->size());

    while (it_boundary != edges2->end() && boundary_count < MAX_BOUNDARY_TEST) {
      // Each position should have consistent property access
      auto current_date =
          it_boundary.property<std::string>("creationDate").value();
      auto repeat_date =
          it_boundary.property<std::string>("creationDate").value();
