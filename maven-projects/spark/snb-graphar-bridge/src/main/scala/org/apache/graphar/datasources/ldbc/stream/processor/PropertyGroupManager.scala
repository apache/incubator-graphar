 ...
 *
 * Responsible for organizing properties into groups according to GraphAr column family specifications, implements:
 * 1. Perform property grouping based on entity type and property characteristics
 * 2. SupportCustomPropertyGroupPolicy
 ...
 */
 ...
 logger.info(s"Property grouping initialization completed: ${propertyGroups.size} groups")
 ...
 */
 ...
 logger.debug(s"Created ${filteredGroups.size} property groups for vertex ${vertex.name}")
 filteredGroups.toMap
 }

 /**
 * Improved field matching algorithm - support case-insensitive and naming convention conversion
 */
 ...
 */
 ...
 case Some(index) =>
 logger.warn(s"Field index out of range: field=$fieldName, index=$index, recordLength=${record.length}")
 ""
 case None =>
 ...
 case Some(index) if index < record.length => record(index)
 case Some(index) =>
 logger.warn(s"Field index out of range: field=$matchedField (matched for $fieldName), index=$index, recordLength=${record.length}")
 ""
 case None =>
 logger.warn(s"Found matching field but unable to get index: matched=$matchedField for original=$fieldName")
 ""
 }
 case None =>
 logger.warn(s"Field not found: $fieldName (available fields: ${allFields.mkString(", ")})")
 ""
 }
 }
 }
