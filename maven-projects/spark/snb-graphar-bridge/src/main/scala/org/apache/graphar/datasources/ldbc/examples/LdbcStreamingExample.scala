...
import org.apache.graphar.datasources.ldbc.bridge.{
  LdbcStreamingBridge,
...
object LdbcStreamingExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      logger.info(
        "Starting LDBC Streaming Bridge Example - Supporting Dynamic Entities"
      )

      if (args.length < 3) {
        logger.error(
          "Usage: LdbcStreamingExample <scale_factor> <output_path> <format> [mode]"
        )
        logger.info(
          "scale_factor: LDBC scale factor (e.g., '0.003', '0.1', '1')"
        )
        ...
        logger.info(
          " LdbcStreamingExample 0.003 /tmp/graphar_streaming/ parquet streaming"
        )
  ...
  def runStreaming(
    ...
    result match {
      case Success(streamingResult) =>
        ...
        logger.info(
          s"Processing Duration: ${streamingResult.processingDurationMs}ms"
        )
        ...
        if (streamingResult.success) {
          ...
          logger.info(
            "✓ Includes: Forum, Post, Comment, Like + Person entities"
          )
  ...
  def runDemo(
    ...
    logger.info(
      "This mode demonstrates the complete streaming processing pipeline"
    )
    ...
    logger.info(
      s"Streaming Mode Entities: ${capability.streamingModeEntities.size}"
    )
  ...
  def runBenchmark(
    ...
    logger.info(
      "Measuring streaming processing performance for complete LDBC dataset"
    )
   ...
   */
  private def validateStreamingConfiguration(
