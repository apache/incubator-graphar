 ...
 *
 * Intercept LDBC RecordOutputStream[StaticGraph.type], convert static entity
 * data to GraphAr format. This class implements architecture symmetry with
 * dynamic entity processing, reusing streaming interception mechanism to
 * process static data.
 *
...
class GraphArStaticOutputStream(
  ...
  @transient private val logger: Logger =
    LoggerFactory.getLogger(classOf[GraphArStaticOutputStream])
  ...
  private val tagClassRelationBuffer =
    mutable.ListBuffer[TagClassIsSubclassOfTagClassData]()
  private val organisationRelationBuffer =
    mutable.ListBuffer[OrganisationIsLocatedInPlaceData]()

  logger.info(
    s"GraphArStaticOutputStream initialized: output_path=$output_path, file_type=$file_type"
  )

  /**
   * Core method for intercepting static entity data write
   *
   * When LDBC generator calls StaticOutputStream.write(StaticGraph), This
   * method will be triggered, we intercept and convert data here
   */
  override def write(staticGraph: StaticGraph.type): Unit = {
    logger.info(
      "=== Starting to intercept static entity data and convert to GraphAr format ==="
    )
  ...
  /**
   * ProcessPlaceEntity Reuse StaticOutputStream.scala:61-67 logic
   */
  private def processPlaceEntities(): Unit = {
    ...
    if (Dictionaries.places == null) {
      logger.error(
        "Dictionaries.places not initialized, cannot process Place entities"
      )
      return
    }
    ...
    if (places == null || places.isEmpty) {
      logger.warn(
        "Places dictionary is empty, skipping Place entity processing"
      )
      return
    }
    ...
    logger.info(
      s"✓ Successfully processed ${placedataBuffer.size} Place entities"
    )
  }

  /**
   * Process Tag entities and TagClass hierarchical structure Reuse
   * StaticOutputStream.scala:69-83 logic
   */
  ...
  /**
   * Process TagClass hierarchical structure Reuse
   * StaticOutputStream.scala:43-58 logic
   */
  private def processTagClassHierarchy(tag: TagData): Unit = {
    ...
    while (classId != -1 && !exportedTagClasses.contains(classId)) {
      ...
      val className =
        StringUtils.clampString(Dictionaries.tags.getClassName(classId), 256)
      ...
      val tagClassData = TagClassData(
        ...
        name = className,
        url =
          if (className == "Thing") "http://www.w3.org/2002/07/owl#Thing"
          else DBPOWL.getUrl(className),
  ...
  /**
   * ProcessOrganisationEntity（Company + University) Reuse
   * StaticOutputStream.scala:85-112 logic
   */
  private def processOrganisationEntities(): Unit = {
    ...
    for { company <- companies } {
      val companyName = StringUtils.clampString(
        Dictionaries.companies.getCompanyName(company),
        256
      )
      val organisationId =
        organisationIdGenerator.getNextId("Organisation", company.toInt)
    ...
    val universities =
      Dictionaries.universities.getUniversities.iterator().asScala
    for { university <- universities } {
      val universityName = StringUtils.clampString(
        Dictionaries.universities.getUniversityName(university),
        256
      )
      val organisationId =
        organisationIdGenerator.getNextId("Organisation", university.toInt)
    ...
    logger.info(
      s"Processed ${organisationdataBuffer.size} Organisation entities"
    )
  }
  ...
  private def generateStaticRelations(): Unit = {
    ...
    logger.info(
      s"Generated static relationships: Place hierarchy ${placeRelationBuffer.size}, " +
  ...
  private def writeStaticEntitiesToGraphAr(): Unit = {
    ...
    try {
      ...
      if (organisationdataBuffer.nonEmpty) {
        ...
        logger.info(
          s"Wrote Organisation vertices: ${organisationdataBuffer.size}"
        )
      }
      ...
      if (placeRelationBuffer.nonEmpty) {
        ...
        logger.info(
          s"Wrote Place hierarchy relationships: ${placeRelationBuffer.size}"
        )
      }
      ...
      if (tagRelationBuffer.nonEmpty) {
        ...
        logger.info(
          s"Wrote Tag classification relationships: ${tagRelationBuffer.size}"
        )
      }
      ...
      if (tagClassRelationBuffer.nonEmpty) {
        writeEdgeData(
          ...
          tagClassRelationBuffer.toSeq
        )
        logger.info(
          s"Wrote TagClass hierarchy relationships: ${tagClassRelationBuffer.size}"
        )
      }
      ...
      if (organisationRelationBuffer.nonEmpty) {
        writeEdgeData(
          ...
          organisationRelationBuffer.toSeq
        )
        logger.info(
          s"Wrote Organisation location relationships: ${organisationRelationBuffer.size}"
        )
      }
   ...
   */
  private def writeVertexData[T <: Product: TypeTag: ClassTag](
      entityType: String,
      data: Seq[T]
  ): Unit = {
    import sparkSession.implicits._
   ...
   */
  private def writeEdgeData[T <: Product: TypeTag: ClassTag](
      relationName: String,
      data: Seq[T]
  ): Unit = {
    import sparkSession.implicits._
  ...
  override def close(): Unit = {
    try {
      ...
      case e: Exception =>
        logger.error(
          "Error occurred while closing GraphArStaticOutputStream",
          e
        )
    }
  }
