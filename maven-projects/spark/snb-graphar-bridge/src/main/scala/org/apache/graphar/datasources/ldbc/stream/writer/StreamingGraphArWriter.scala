 ...
 *
 * Responsible for writing processed streaming data to GraphAr format,
 * implements:
 *   1. Shard writes according to chunk size 2. Generate GraphAr standard
 *      metadata files 3. Support multiple file formats (csv/parquet/orc) 4.
 *      Maintain offset information and indices
 */
class StreamingGraphArWriter(
  ...
  private val logger: Logger =
    LoggerFactory.getLogger(classOf[StreamingGraphArWriter])
  ...
  private val offsetmappings =
    mutable.Map[String, mutable.Map[String, mutable.Map[Long, OffsetInfo]]]()
  ...
  private val schemamapping =
    mutable.Map[String, EntitySchemaDetector.EntitySchema]()

  logger.info(
    s"StreamingGraphArWriter initialized: output_path=$output_path, graph_name=$graph_name, using GraphWriter mode"
  )

  /**
   * Get path conforming to GraphAr standard According to GraphAr specification,
   * vertex data should be in vertex/ directory, edge data should be in edge/
   * directory
   */
  ...
  def writeEntity(
    ...
    logger.info(
      s"Writing entity data in streaming mode: entity=$entityName, propertyGroup=$propertyGroup, rows=${dataFrame.count()}"
    )
    ...
    try {
      ...
      // Initializecounter
      val entityCounters =
        chunkCounters.getOrElseUpdate(entityName, mutable.Map[String, Long]())
      val currentChunkId = entityCounters.getOrElse(propertyGroup, 0L)

      // Determine chunk size
      val chunkSize =
        if (entityType.isVertex) vertex_chunk_size else edge_chunk_size
      ...
      case e: Exception =>
        logger.error(
          s"Failed to write entity data in streaming mode: entity=$entityName, propertyGroup=$propertyGroup",
          e
        )
        WriteResult(
          success = false,
   ...
   */
  def generateGraphMetadata(
      schemas: List[EntitySchemaDetector.EntitySchema]
  ): Unit = {
    logger.info(
      "Using standard GraphWriter to generate GraphAr format data and metadata files"
    )
    ...
    try {
        ...
        schemamapping.get(collectKey) match {
          case Some(schema) =>
            ...
            if (entityType.isVertex) {
              ...
              logger.info(
                s"Preparing to write vertex data: $vertexType, record count: ${dataFrame.count()}"
              )
              ...
              logger.info(
                s"Preparing to write edge data: ${relation._1}->${relation._2}->${relation._3}, record count: ${dataFrame.count()}"
              )
            }
      ...
      edgeDataFrames.foreach { case (relation, dataFrame) =>
        ...
        logger.info(
          s"Adding edge data to GraphWriter: ${relation._1}->${relation._2}->${relation._3}"
        )
      }
      ...
      logger.info(
        s"GraphWriter write completed: output_path=$output_path, graph_name=$graph_name"
      )

    } catch {
      case e: Exception =>
        logger.error(
          "Failed to generate data and metadata files using GraphWriter",
          e
        )
        throw e
    }
  ...
  private def writeChunk(
    ...
    try {
      ...
      logger.debug(
        s"Successfully wrote chunk: $chunkPath, row count: $rowCount"
      )
  ...
  private def recordOffsetInfo(
    ...
    val entityOffsets = offsetmappings.getOrElseUpdate(
      entityName,
      mutable.Map[String, mutable.Map[Long, OffsetInfo]]()
    )
    val groupOffsets = entityOffsets.getOrElseUpdate(
      propertyGroup,
      mutable.Map[Long, OffsetInfo]()
    )
  ...
  private def updateEntityStatistics(
    ...
    val stats = entityStatistics.getOrElseUpdate(
      entityName,
    ...
    val groupStats = stats.propertyGroups.getOrElseUpdate(
      propertyGroup,
      PropertyGroupStats(
        propertyGroup = propertyGroup,
        rowCount = 0L,
    ...
    offsetmappings.foreach { case (entityName, entityOffsets) =>
      entityOffsets.foreach { case (propertyGroup, groupOffsets) =>
        ...
        try {
          ...
          writer.write(
            "# Format: chunk_id,start_offset,end_offset,row_count\\n"
          )

          groupOffsets.toSeq.sortBy(_._1).foreach {
            case (chunkId, offsetInfo) =>
              writer.write(
                s"${offsetInfo.chunkId},${offsetInfo.startOffset},${offsetInfo.endOffset},${offsetInfo.rowCount}\\n"
              )
          }
