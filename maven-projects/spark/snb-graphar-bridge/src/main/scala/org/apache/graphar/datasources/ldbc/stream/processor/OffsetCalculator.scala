 ...
 *
 * GraphAr uses chunked storage to optimize large-scale graph data access
 * performance:
 *   1. Divide large amounts of data into fixed-size chunks 2. Data within each
 *      chunk is stored contiguously, improving locality 3. Support parallel
 *      access to different chunks 4. Facilitate memory management and I/O
 *      optimization
 *
 * For vertex data: chunk_size is typically set to 1024-4096 For edge data:
 * chunk_size can be set larger, such as 8192-16384
 */
class OffsetCalculator(chunkSize: Long) {

  private val logger: Logger =
    LoggerFactory.getLogger(classOf[OffsetCalculator])
   ...
   *
   * @param internalId
   *   Continuous internal ID (starting from 0)
   * @return
   *   (chunkIndex, offset) tuple
  ...
  def calculateOffset(internalId: Long): (Int, Long) = {
    require(
      internalId >= 0,
      s"Internal ID must be non-negative, got: $internalId"
    )
  ...
  def calculateInternalId(chunkIndex: Int, offset: Long): Long = {
    require(
      chunkIndex >= 0,
      s"Chunk index must be non-negative, got: $chunkIndex"
    )
    require(
      offset >= 0 && offset < chunkSize,
      s"Offset must be in range [0, $chunkSize), got: $offset"
    )
   ...
   *   ChunkIndex
   * @return
   *   (startId, endId) ID range contained in this chunk (includes startId,
   *   excludes endId)
   */
  ...
  def calculateChunkCount(entityCount: Long): Int = {
    require(
      entityCount >= 0,
      s"Entity count must be non-negative, got: $entityCount"
    )
  ...
  def getChunkFillRatio(chunkIndex: Int, totalEntities: Long): Double = {
    ...
    require(
      totalEntities >= 0,
      s"Total entities must be non-negative, got: $totalEntities"
    )
  ...
  def generateChunkStatistics(totalEntities: Long): ChunkStatistics = {
    ...
    val fullChunks =
      if (totalEntities % chunkSize == 0) totalChunks else totalChunks - 1
    val partialChunks = totalChunks - fullChunks
    val lastChunkSize =
      if (totalEntities % chunkSize == 0) chunkSize
      else totalEntities % chunkSize
   ...
   */
  def validateChunkConfiguration(
      maxExpectedEntities: Long
  ): ValidationResult = {
  ...
  def logChunkStatistics(statistics: ChunkStatistics): Unit = {
    ...
    logger.info(
      s"Average fill ratio: ${(statistics.averageFillRatio * 100).formatted("%.1f")}%"
    )
  }
