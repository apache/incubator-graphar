...
/**
 * GraphAr output stream, extends LDBC RecordOutputStream interface Process
 * string column table data and convert to GraphAr format
 */
class GraphArActivityOutputStream(
    ...
    extends RecordOutputStream[JList[String]]
    with Serializable {

  @transient private val logger: Logger =
    LoggerFactory.getLogger(classOf[GraphArActivityOutputStream])
  ...
  private def getStandardGraphArPath(entityType: String): String = {
    entityType.toLowerCase match {
      // VertexEntity
      case "person" | "forum" | "post" | "comment" | "photo" =>
        s"$output_path/vertex/$entityType"

      // Edge entity - automatically detect edge entity name containing underscores in package
      case edgeType
          if edgeType.contains("_") && !edgeType.startsWith("vertex/") =>
  ...
  /**
   * Process real LDBC activity data (based on ActivityoutputStream
   * architecture)
   */
   ...
   */
  private def writePostWall(
      genWall: GenWall[Triplet[
        Post,
        java.util.stream.Stream[Like],
        java.util.stream.Stream[Pair[Comment, java.util.stream.Stream[Like]]]
      ]]
  ): Unit = {
    import scala.collection.JavaConverters._
    ...
    genWall.inner.forEach { triplet =>
      ...
      val posts =
        triplet.getValue2 // Stream<Triplet<Post, Stream<Like>, Stream<Pair<Comment, Stream<Like>>>>>
      ...
      posts.forEach { postTriplet =>
        ...
        val comments =
          postTriplet.getValue2 // Stream<Pair<Comment, Stream<Like>>>
   ...
   */
  private def writeAlbumWall(
      genAlbums: GenWall[Pair[Photo, java.util.stream.Stream[Like]]]
  ): Unit = {
    import scala.collection.JavaConverters._
  ...
  private def writeComment(comment: Comment): Unit = {
    ...
    val commentRecord = List(
      ...
      if (comment.getRootPostId != -1) comment.getRootPostId.toString
      else "", // replyToPostId
      if (
        comment.getParentMessageId != -1 && comment.getParentMessageId != comment.getRootPostId
      ) comment.getParentMessageId.toString
      else "" // replyToCommentId
    ).asJava
  ...
  private def writeCommentReplyOfEdges(comment: Comment): Unit = {
    ...
    if (rootPostId != -1) {
      if (parentMessageId == -1 || parentMessageId == rootPostId) {
        ...
        val edgeHeader =
          List("commentId", "replyToCommentId", "creationDate").asJava
        val edgeRecord = List(
          comment.getMessageId.toString,
  ...
  private def writePostLike(like: Like): Unit = {
    ...
    logger.debug(
      s"Writing Person_likes_Post: Person ${like.getPerson} likes Post ${like.getMessageId}"
    )
  ...
  private def writeCommentLike(like: Like): Unit = {
    ...
    logger.debug(
      s"Writing Person_likes_Comment: Person ${like.getPerson} likes Comment ${like.getMessageId}"
    )
  ...
  /**
   * Write Person_likes_Post edge（Photo likes) Fix: Photo likes should write
   * Person_likes_Post, not Person_likes_Photo
   */
  private def writePhotoLike(like: Like): Unit = {
    import scala.collection.JavaConverters._

    logger.debug(
      s"Writing Person_likes_Post (Photo): Person ${like.getPerson} likes Photo ${like.getMessageId}"
    )
   ...
   * Write Forum membership relationship
   */
  private def writeForumMembership(
      membership: ldbc.snb.datagen.entities.dynamic.relations.ForumMembership
  ): Unit = {
    import scala.collection.JavaConverters._

    logger.debug(
      s"Writing Forum membership: Person ${membership.getPerson.getAccountId} member of Forum ${membership.getForumId}"
    )
  ...
  /**
   * Write Photo entity (fix: Photo as Post imageFile property, not standalone
   * vertex) Reference LDBC official:ActivityoutputStream.scala:133-155
   */
  ...
  /**
   * Unified entity record processing method (direct processing, avoid recursive
   * creation)
   */
  private def processEntityRecord(
      entityType: String,
      header: JList[String],
      record: JList[String]
  ): Unit = {
    // Get or initialize the entity type data buffer
    val entityBuffer = entityBuffers.getOrElseUpdate(
      entityType,
      mutable.ArrayBuffer[JList[String]]()
    )
    ...
    if (entityHeaderMap.isEmpty) {
      ...
      logger.debug(
        s"Set header for $entityType entity: ${header.asScala.mkString(", ")}"
      )
    }
    ...
    entityBuffer += record
    logger.debug(
      s"Adding $entityType entity record, current buffer size: ${entityBuffer.size}"
    )
  ...
  private def flushEntityBuffer(entityType: String): Unit = {
    ...
    if (buffer.isDefined && headerOpt.isDefined && buffer.get.nonEmpty) {
      try {
        logger.info(
          s"Flushing $entityType entity buffer, record count: ${buffer.get.size}"
        )
        ...
        case e: Exception =>
          logger.error(
            s"Error occurred while flushing $entityType entity buffer",
            e
          )
   ...
   */
  private def writeEntityToGraphAr(
    ...
    logger.debug(
      s"Writing $entityType entity to GraphAr, header: ${headerList.mkString(", ")}"
    )
    ...
    logger.info(
      s"Successfully wrote $entityType entity ${records.size} records to $entityPath"
    )
  }

  // Entity data buffer (grouped by entity type)
  private val entityBuffers =
    mutable.Map[String, mutable.ArrayBuffer[JList[String]]]()
  ...
  private def processBatch(): Unit = {
    if (
      dataBuffer.isEmpty || headerBuffer.isEmpty || entityType.isEmpty || detectedSchema.isEmpty
    ) {
      return
    }
    ...
    try {
      ...
      batchCounter += 1
      logger.info(
        s"Successfully processed batch $batchCounter, record count: ${dataBuffer.size}"
      )
      dataBuffer.clear()
    } catch {
      case e: Exception =>
  ...
  private def writeToGraphAr(
    ...
    val structFields = groupFields.map { fieldName =>
      val dataType = schema
        ...
        .getOrElse(org.apache.spark.sql.types.StringType)
    ...
    logger.info(
      s"Final StructType field order: ${structType.fieldNames.mkString(", ")}"
    )
    ...
    val rows = records.map { record =>
      ...
      val orderedValues = groupFields.map { fieldName =>
        headerToIndexMap.get(fieldName) match {
          ...
          case Some(index) =>
            logger.warn(
              s"Field $fieldName index $index exceeds record length ${recordArray.length}"
            )
            null
          case None =>
  ...
  private def writeDataFrameToGraphAr(
    ...
    logger.info(
      s"Writing $entityType entity to GraphAr format, record count: ${df.count()}"
    )
  ...
  def getStatistics(): StreamingStatistics = {
    StreamingStatistics(
      recordCount =
        dataBuffer.size + batchCounter * 1000, // Estimate total record count
      processedCount = batchCounter * 1000,
      batchCount = batchCounter,
      entityType = entityType.getOrElse("Unknown"),
      isValidSchema =
        detectedSchema.isDefined && detectedSchema.get.isValidSchema
    )
  }
  ...
  override def close(): Unit = {
    try {
      ...
      logger.info(
        s"Processed entity types: ${entityBuffers.keys.mkString(", ")}"
      )
...
object GraphArActivityOutputStream {
  ...
  )(implicit spark: SparkSession): GraphArActivityOutputStream = {
