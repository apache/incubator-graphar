...
TEST_CASE("test_pagerank_example") {
  ...
  GAR_NAMESPACE::Property pagerank = {"pagerank",
                                      GAR_NAMESPACE::DataType(GAR_NAMESPACE::Type::DOUBLE), false};
  ...
  REQUIRE(extend_info.Save("/tmp/person-new-pagerank.vertex.yml").ok());
  ...
  REQUIRE(array_builder.AppendValues(pr_curr).ok());
